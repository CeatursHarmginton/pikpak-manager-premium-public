...
## 🛠️ Technical & Development Guide (For Developers)

If you wish to run PikPak Manager from source, modify the code, or bundle your own executable, please follow the guide below.

### 1. Requirements & Running from Source
...
python -m uvicorn backend.app.main:app --host 127.0.0.1 --port 8765 --reload --loop asyncio
```

Open `http://127.0.0.1:8765` in your browser.
*   **Convenience shortcut**: You can run the bundled `run.bat` which maps the host to `http://ppmanager.com:8765`. Run `setup-domain.bat` once as Administrator to register this domain in your Windows hosts file.

---

### 2. Building the Windows Portable EXE
To package the entire backend FastAPI, frontend static assets, and the Python runtime into a single, clean `.exe` application with system tray support:

```powershell
.\build-exe.bat
```

*   The output executable will be compiled to `dist/PikPak Manager.exe`.
*   To build a troubleshooting executable that includes a visual command console for debugging:
    ```powershell
    .\build-exe.bat --debug-console
    ```
*   You can set the environment variable `PIKPAK_DESKTOP_DEBUG=1` before launching the executable to write logs to `data/desktop.log`.

---

### 3. File Integrity & Premium Features Enforcements
Premium Vault actions (`/api/vault/*`) and licensing rely on the internal Premify session verification. 
Before packaging a production build, generate the file integrity manifest:

```powershell
python scripts/generate_integrity_manifest.py
```

Provide your deployment secrets via environment variables:
*   `PIKPAK_PREMIFY_API_KEY`
*   `PIKPAK_PREMIFY_API_SECRET`
*   `PIKPAK_INTEGRITY_SECRET` (used to sign and verify the generated file manifest integrity at runtime).
